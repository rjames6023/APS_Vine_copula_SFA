function [theta, sterr, u_hat, non_par_u_hat, u_hat_conditional_w1w2] = nested_AppLoglikelihood_Vine_APS3A(y, x1, x2, z1, z2, us, theta0, random_seed)
    Options = optimset('TolX', 1e-8, 'TolFun', 1e-8, 'MaxIter', 20000, 'MaxFunEvals', 6 * 20000, 'Display', 'Off');
     [theta, logMLE, grad, hessian] = fminunc(@AppLoglikelihood_Vine_APS3A, theta0, Options);
...
%     problem = createOptimProblem('fmincon','x0',theta0, 'objective',@AppLoglikelihood_Vine_APS3A,'options',Options, ...
%                                  'lb',[0, 0, 0, log(0.1), log(0.1), log(0.1), log(0.1), 0.1, -0.49, -0.49, -0.99], ...
%                                  'ub',[1, 1, 1, log(1.5), log(1.5), log(1.5), log(1.5), 1.5, 0.49, 0.49, 0.99]);
...
%     theta(10:11) = ((b-a)/2).*tanh(theta(10:11)) + (b+a)/2;
    ...
    %Estimate standard errors using the OPG
...
%     hessian = hessian(:,1:hessian_P-3);
    ...
    for i=1:length(theta)-3
        ...
        grad_contributions(:,i) = (AppLogDen_Vine_APS3A(theta1, 0)-AppLogDen_Vine_APS3A(theta, 0))/delta; 
    end
    ...
    rep_obs_w2 = reshape(repelem(obs_w2, S_kernel), S_kernel, n);
        ...
        %Sample from the APS3A Vine Copula using conditional sampling
    APS2A_vine_copula_samples = zeros(S_kernel,3);
    %Partial derivative of Gaussian Copula w.r.t second argument
    C12_condtional_2_inverse_ = @(u1_, u2_, pw1w2_) normcdf((norminv(u1_) - pw1w2_*norminv(u2_))/(sqrt(1 - pw1w2_^2)));
        %Partial derivative of APS2A copula w.r.t second argument (conditional cdf)
    % syms u1 u2 theta
    % APS2A_CDF = u1*u2 + theta*u1*u2*(1-u1)*(1 - (4*u2^2 - 6*u2 + 3));
    % PD = diff(APS2A_CDF, u2);
    copula_PDs_matrix = {C12_condtional_2_inverse_ @APS2A_conditional_u2; @APS2A_conditional_u2 0};
    ...
    for h = 1:S_kernel
        ...
        APS2A_vine_copula_samples(h,:) = [X_vector(3), X_vector(1), X_vector(2)];
    end
    rng(random_seed) %Set seed
    simulated_v = normrnd(0, sqrt(sigma2v), S_kernel, 1);
    simulated_u = sqrt(sigma2u)*norminv((APS2A_vine_copula_samples(:,1)+1)/2, 0,1); %simulated half normal rvs
    simulated_w1 = norminv(APS2A_vine_copula_samples(:,2), 0, sqrt(sigma2w1));
    simulated_w2 = norminv(APS2A_vine_copula_samples(:,3), 0, sqrt(sigma2w2));
    ...
    function logL = AppLoglikelihood_Vine_APS3A(coefs)
        % tranform parameters back true range
        coefs(4:7) = exp(coefs(4:7));
%         b = 1;
%         a = -1;
%         coefs(10:11) = ((b-a)/2).*tanh(coefs(10:11)) + (b+a)/2;
        logDen = AppLogDen_Vine_APS3A(coefs, 1);
        logL = -sum(logDen);
    end

    function logDen = AppLogDen_Vine_APS3A(coefs, check_theta_bounds) 
        % tranform parameters back true range
        N = length(y); %n obs
        ...
        if ((theta12 - 1e-5 < -0.5 || theta12 + 1e-5 > 0.5) || (theta13 - 1e-5 < -0.5 || theta13 + 1e-5 > 0.5)) && check_theta_bounds == 1
            ...
            if flag ~= 0
                ...
                conditional_CDF_w1_u = CDF_w1_rep + theta12.*CdfUs.*CDF_w1_rep.*(4.*CDF_w1_rep.^2 - 6.*CDF_w1_rep + 2) + theta12.*CDF_w1_rep.*(CdfUs - 1).*(4*CDF_w1_rep.^2 - 6.*CDF_w1_rep + 2);
                conditional_CDF_w2_u = CDF_w2_rep + theta13.*CdfUs.*CDF_w2_rep.*(4.*CDF_w2_rep.^2 - 6.*CDF_w2_rep + 2) + theta13.*CDF_w2_rep.*(CdfUs - 1).*(4*CDF_w2_rep.^2 - 6.*CDF_w2_rep + 2);
                for j = 1:N
                    simulated_c21_pdf = 1 + theta12.*(1-2.*CdfUs(:,j)).*(1 - 12.*(CDF_w1_rep(:,j) - 0.5).^2);
                    simulated_c31_pdf = 1 + theta13.*(1-2.*CdfUs(:,j)).*(1 - 12.*(CDF_w2_rep(:,j) - 0.5).^2);
                    simulated_c23_pdf = copulapdf('Gaussian',[conditional_CDF_w1_u(:,j), conditional_CDF_w2_u(:,j)],RhoWW);
                    APS3A_copula_pdf = simulated_c21_pdf.*simulated_c31_pdf.*simulated_c23_pdf;
