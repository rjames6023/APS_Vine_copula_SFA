function [theta, sterr, u_hat, non_par_u_hat, u_hat_conditional_w1w2] = nested_AppLoglikelihood_APS3A(y, x1, x2, z1, z2, us, theta0, random_seed)
    ...
    [theta, logMLE, grad, hessian] = fminunc(@AppLoglikelihood_APS3A, theta0, Options);
...
%      problem = createOptimProblem('fmincon','x0',theta0, 'objective',@AppLoglikelihood_APS3A,'options',Options, ...
...
%     hessian = hessian(:,1:hessian_P-3);
    ...
    for i=1:length(theta)-3
        ...
        grad_contributions(:,i) = (AppLogDen_APS3A(theta1, 0)-AppLogDen_APS3A(theta, 0))/delta; 
    end
    ...
    for h=1:S_kernel
        ...
        c12_deriv_u1_inverse_func = @(u2) c12_deriv_u1(x1, u2, theta12) - PPP(2);
        ...
        x3_inverse_function = @(u3) (c13_int(x1, u3, theta13) + int_c23(u3) - u3) - PPP(3);
        ...
        APS3A_copula_samples(h,:) = [x1, x2, x3];
    end
    ...
    simulated_w1 = norminv(APS3A_copula_samples(:,2), 0, sqrt(sigma2w1));
    simulated_w2 = norminv(APS3A_copula_samples(:,3), 0, sqrt(sigma2w2));
    ...
    u_hat_conditional_w1w2 = kernel_regression_results;
    
    function logL = AppLoglikelihood_APS3A(coefs)
        %tranform parameters back true range
        coefs(4:7) = exp(coefs(4:7));
        logDen = AppLogDen_APS3A(coefs, 1);
        logL = -sum(logDen);
    end

    function logDen = AppLogDen_APS3A(coefs, check_theta_bounds) 
        N = length(y); %n obs
        ...
        if ((abs(theta12) + abs(theta13) + 1e-5) > (delta_bound/2)) && check_theta_bounds == 1 %Add a tolerance to ensure standard errors can be estimated using OPG
            ...
            %Construct technical inefficiency sample information
            ...
            if flag ~= 0
                ...
                for j = 1:N
                    simulated_c12_pdf = 1 + theta12.*(1-2.*CdfUs(:,j)).*(1 - 12.*(CDF_w1_rep(:,j) - 0.5).^2);
                    simulated_c13_pdf = 1 + theta13.*(1-2.*CdfUs(:,j)).*(1 - 12.*(CDF_w2_rep(:,j) - 0.5).^2);
