function [logMLE, theta, sterr, u_hat, V_u_hat, non_par_u_hat, u_hat_conditional_w2w3, V_u_hat_conditional_w2w3] = nested_AppLoglikelihood_Vine_APS3A(y, x1, x2, x3, p1, p2, p3, us, theta0)
    Options = optimset('TolX', 1e-8, 'TolFun', 1e-8, 'MaxIter', 20000, 'MaxFunEvals', 6 * 20000, 'Display', 'Off');
    [theta, logMLE] = fminunc(@AppLoglikelihood_Vine_APS3A, theta0, Options); %Maximise the log-likelihood
...
%    theta(12:13) =((b-a)/2)*((2/pi).*atan(theta(12:13))) + (b+a)/2;
    ...
    for i=1:length(theta)
        ...
        grad(:,i) = (AppLogDen_Vine_APS3A(theta1, 0) - AppLogDen_Vine_APS3A(theta, 0))/delta; 
    end
    ...
    simulated_w2 = norminv(APS2A_vine_copula_samples(:,2), mu2, sqrt(sigma2w2));
    simulated_w3 = norminv(APS2A_vine_copula_samples(:,3), mu3, sqrt(sigma2w3));
    ...
    V_u_hat_conditional_w2w3 = kernel_regression_variance_results;
...
%         coefs(12:13) =((b-a)/2)*((2/pi).*atan(coefs(12:13))) + (b+a)/2;
    ...
    function logDen = AppLogDen_Vine_APS3A(coefs, check_theta_bounds) 
        ...
        if ((theta12 - 1e-5 < -0.5 || theta12 + 1e-5 > 0.5) || (theta13 - 1e-5 < -0.5 || theta13 + 1e-5 > 0.5)) && check_theta_bounds == 1 %Add a tolerance to ensure standard errors can be estimated using OPG
            ...
            if flag ~= 0 
                ...
                %conditional_CDF_w2_u = CDF_w2_rep + (theta12*CDF_w2_rep).*(1 - CDF_w2_rep).*(2*(6*CdfUs - 6*CdfUs.^2 - 1));
                %conditional_CDF_w3_u = CDF_w3_rep + (theta13*CDF_w3_rep).*(1 - CDF_w3_rep).*(2*(6*CdfUs - 6*CdfUs.^2 - 1));
