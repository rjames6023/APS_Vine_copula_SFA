function [logMLE, theta, sterr, u_hat, V_u_hat, non_par_u_hat, u_hat_conditional_w2w3, V_u_hat_conditional_w2w3] = nested_AppLoglikelihood_APS3B(y, x1, x2, x3, p1, p2, p3, us, theta0)
    Options = optimset('TolX', 1e-10, 'TolFun', 1e-10, 'MaxIter', 20000, 'MaxFunEvals', 6 * 20000, 'Display', 'Off');
    [theta, logMLE] = fminunc(@AppLoglikelihood_APS3B, theta0, Options); %Maximise the log-likelihood
...
%     [theta, logMLE] = run(gs, problem);    
    ...
    for i=1:length(theta)
        ...
        grad(:,i) = (AppLogDen_APS3B(theta1, 0) - AppLogDen_APS3B(theta, 0))/delta; 
    end
...
%     c13_int = int(f_c13, u3, 0, u3);
    ...
    for h=1:S_kernel
        ...
        c12_deriv_u1_inverse_func = @(u2) APSA3B_conditional_u1(z1, u2, theta12) - PPP(2);
        ...
        x3_inverse_function = @(u3) (APSA3B_integral_u3(z1, u3, theta13) + int_c23(u3) - u3) - PPP(3);
        z3 = fzero(x3_inverse_function, [0 1]);

        APS3B_copula_samples(h,:) = [z1, z2, z3];
    end
    ...
    simulated_w2 = norminv(APS3B_copula_samples(:,2), mu2, sqrt(sigma2w2));
    simulated_w3 = norminv(APS3B_copula_samples(:,3), mu3, sqrt(sigma2w3));
